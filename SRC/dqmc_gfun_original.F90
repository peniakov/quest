...
#include "dqmc_include.h"
  ...
  type G_fun
     ...
     ! For numerical stab
     integer  :: nWrap, wps, lastwr, maxwrap
...
contains
  ...
  subroutine DQMC_GFun_Init(n, L, G, V, WS, nWrap, difflim, errrate, up, ssxx)
    !
    ! Purpose
    ! =======
  ...
  subroutine DQMC_GFun_Init_original(n, L, G, V, WS, nWrap, difflim, errrate, up, ssxx)
    !
    ! Purpose
    ! =======
    !    This subroutine initiliazes the data type of Green function.
  ...
  subroutine DQMC_ComputeG(il, n, sgn, G, V, SB, pvt2, compDet, det, sxx)
    ...
    if (SB%svd) then
       ...
       !compute T U = transpose(inv(U)inv(T))
       call blas_dgemm('N','N', n, n, n, ONE, T, n, U, n, ZERO, W1, n)

       !Compute sign of determinant of T U
       call blas_dcopy(n*n, W1, 1, G, 1)  
       call lapack_dgetrf(n, n, G, n, pvt1, info)
       call DQMC_DetSgn(n, G, pvt1, sgn)
    ...
    if (SB%svd) then
       ...
       call blas_dgemm('T','N', n, n, n, ONE, T, n, G, n, ZERO, W1, n)
       call blas_dcopy(n*n, W1, 1, G, 1)  
  ...
  subroutine DQMC_UpdateWraps(G)
    ...
    write(*,'(3x,A,i4,A,i6)') 'Total number of errors : ', G%redo,' out of ', G%noredo+G%redo
    ...
    if(G%NoRedo .gt. DQMC_NOREDO_F) then
       if(redorat .lt. DQMC_REDO_RATE*G%errrate) then
          if (G%nwrap .ge. G%lastwr) then
             ...
             G%nwrap = min(G%nwrap + 1, G%maxwrap)
             G%redo = 0
             G%noredo = 1
             G%lastwr = G%nwrap
          end if
